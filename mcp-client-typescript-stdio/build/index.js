...
function generateRandomBitBuffer(bitLength) {
...
function formatBits(bits) {
    if (bits >= 8_589_934_592)
        return (bits / 8_589_934_592).toFixed(2) + " GB";
    if (bits >= 8_388_608)
        return (bits / 8_388_608).toFixed(2) + " MB";
    if (bits >= 8_192)
        return (bits / 8_192).toFixed(2) + " kB";
    if (bits >= 8)
        return (bits / 8).toFixed(2) + " B";
    return bits + " bits";
}
class MCPClient {
    messages = [
    ...
    async runBenchmark() {
        ...
        for (const size of sizes) {
            ...
            try {
                ...
                console.log(`Size: ${size.toString().padStart(10)} bytes | Time: ${(end - start).toFixed(4).padStart(10)} ms | Valid: ${isValid}`);
            }
            catch (err) {
    ...
    async runBitLevelBenchmark() {
        ...
        for (const bits of bitSizes) {
            ...
            try {
                ...
                const echoedBase64 = result?.content?.[0]?.text ?? "";
                const echoedBuffer = Buffer.from(echoedBase64, "base64");
                const end = performance.now();
                const isValid = buffer.equals(echoedBuffer);
                console.log(`Bits: ${formatBits(bits).padStart(10)} | Time: ${(end - start).toFixed(4).padStart(10)} ms | Mbps: ${((bits / (end - start) / 1000000)).toFixed(8).padStart(10)} mbps | Valid: ${isValid}`);
                if (!isValid) {
